...
import { DimensionType } from "../Calc/Calc";
...
export const Dimension: FC<DimensionPropsType> = (props) => {
	const { dimensionConfig, setSelectedWidth, setSelectedLength, setIsErrorOccured } = props;
	...
	const isValidInputValue = (value: number, min: number, max: number): boolean => {
		return (value < min || value > max || isNaN(value));
	}

	const onHandleChangeWidth = (e: ChangeEvent<HTMLInputElement>) => {
		let curWidth: number = +e.target.value;
		const isErrorWidth = isValidInputValue(curWidth, dimensionConfig.width.min, dimensionConfig.width.max);
	...
	const onHandleChangeLength = (e: ChangeEvent<HTMLInputElement>) => {
		let curLength: number = +e.target.value;
		const isErrorLength = isValidInputValue(curLength, dimensionConfig.length.min, dimensionConfig.length.max);
	...
	return (
		<div className={s["dimension-container"]}>
			<div className={s["dimension-container-item"]}>
				<span>Длина:</span>
				<input type="number" value={length} onChange={onHandleChangeLength}
					className={isLengthError ? s["dimension-container-item__input_error"] : s["dimension-container-item__input"]}
					max={dimensionConfig.length.max} min={dimensionConfig.length.min}
					step={dimensionConfig.length.step} />
			</div>
			<div className={s["dimension-container-item"]}>
				<span>Ширина:</span>
				<input type="number" value={width} onChange={onHandleChangeWidth}
					className={isWidthError ? s["dimension-container-item__input_error"] : s["dimension-container-item__input"]}
					max={dimensionConfig.width.max} min={dimensionConfig.width.min}
